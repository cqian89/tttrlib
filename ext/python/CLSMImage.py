...
def __init__(
    ...
    **kwargs,
    ...
    settings_kwargs = {
        ...
        "marker_line_start": int(marker_line_start)
        if marker_line_start is not None
        else 3,
        "marker_line_stop": int(marker_line_stop)
        if marker_line_stop is not None
        else 2,
        "marker_frame_start": marker_frame_start
        if marker_frame_start is not None
        else [1],
        "marker_event_type": int(marker_event_type)
        if marker_event_type is not None
        else 1,
        "n_pixel_per_line": int(n_pixel_per_line)
        if n_pixel_per_line is not None
        else 1,
        "bidirectional_scan": False,  # <-- new default
        "split_by_channel": bool(split_by_channel),
        "use_pixel_markers": bool(use_pixel_markers),
        "marker_pixel": int(marker_pixel) if marker_pixel is not None else 1,
    }
